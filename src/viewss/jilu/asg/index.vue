<template>
    <div class="bar">
         <div style="width: 500px;height: 70px;display:flex; flex-direction:row; ">
            <div><el-input placeholder="搜索钱包地址" prefix-icon="el-icon-search" v-model="sousuo" @input='SouSuo(1)' clearable></el-input></div>
            <div style="width: 20px;" ></div>
            <div><el-select v-model="sousuo1" placeholder="全部"><el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value"></el-option></el-select></div>
        </div>
        <el-table :data="juluData" border style="width: 100%">
            ...
            <!-- 消耗方式 -->
            <el-table-column  label="消耗方式" align="center">
                <template slot-scope="scope" >
                    <div style="margin-top: 5px;">{{ scope.row.logTypeTxt }}</div>
                </template>
            </el-table-column>
             <!-- 消耗详情 -->
...
<script>
 import {logAsg} from "@/api/user";
    ...
    created() {
        ...
        // this.sousuo = this.$route.params.address;
    },
    methods: {
        //获取饲料配置
        async GetUseInfo() {
            ...
            const data = await logAsg(_paget)
