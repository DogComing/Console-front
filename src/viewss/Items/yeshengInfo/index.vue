<template>
    <div class="bar">
        ...
        <el-table :data="userData" border style="width: 100%">
            ...
            <!-- 战力 -->
            <el-table-column prop="ags" label="增加战力" align="center">
                <template slot-scope="scope" >
            ...
            <!-- 是否不受天赋影响 -->
            <el-table-column prop="ags" label="天赋影响" width="130" align="center">
                <template slot-scope="scope" >
                    <el-switch
...
<script>
    import {wildAll,wildUpdate} from "@/api/user";
    export default {
    name: 'UserInfo',
    ...
    methods: {
        ...
        async GetUseInfo() {
            ...
            //     }
            // }
            const data = await wildAll(_paget)
            //修改页
            ...
            for (let i = 0; i < data.data.data.length; i++) {
                let value ={
                    ...
                    //名字
                    name: {
                            value: data.data.data[i].wildName, 
                            setValue:'',
                            isSet:false,
                        },
                    //说明
                    introduce:{
                        value: data.data.data[i].wildDesc, 
                        setValue:'',
                        isSet:false,
                    },
                    //战力
        ...
        SetPinZhi(index, row) {
            ...
            else{ _paget['dogName']=this.userData[index].name.setValue;}}
            if(this.userData[index].introduce.isSet){if(this.userData[index].introduce.setValue==''){this.$message.error('修改失败\n空');return;}
            else{ _paget['wildDesc']=this.userData[index].introduce.setValue;}} 
